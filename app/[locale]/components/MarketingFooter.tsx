...
export default async function MarketingFooter() {
    ...
    // Logic to ensure "Resources" (or "Support") section exists with University and Docs links
    let sections = [...dbSections];
    const resourceSectionIndex = sections.findIndex(s => s.title === "Resources" || s.title === "Support");

    if (resourceSectionIndex !== -1) {
        // Section exists, append links if not present
        const section = sections[resourceSectionIndex];
        const hasDocs = section.links.some(l => l.url === "/docs");
        const hasUniversity = section.links.some(l => l.url === "/sop");

        const newLinks = [...section.links];
        if (!hasDocs) newLinks.push({ id: "docs-static", text: "Documentation", url: "/docs", order: 998 } as any);
        if (!hasUniversity) newLinks.push({ id: "university-static", text: "University", url: "/sop", order: 999 } as any);

        // @ts-ignore
        sections[resourceSectionIndex] = { ...section, links: newLinks };
    } else {
        // Section does not exist, create "Resources" section
        sections.push({
            ...
            links: [
                ...
                { id: "university-static", text: "University", url: "/sop", order: 2 }
            ]
        } as any);
    }
    // Logic to ensure "Company" section exists and has Newsletter link
    const companySectionIndex = sections.findIndex(s => s.title === "Company");
    if (companySectionIndex !== -1) {
        const section = sections[companySectionIndex];
        const hasNewsletter = section.links.some(l => l.text === "Newsletter");

        if (!hasNewsletter) {
            const newLinks = [...section.links];
            newLinks.push({ id: "newsletter-static", text: "Newsletter", url: "?newsletter=true", order: 999 } as any);
            // @ts-ignore
            sections[companySectionIndex] = { ...section, links: newLinks };
        }
    } else {
        // Create Company section if missing (unlikely but safe)
        sections.push({
            id: "generated-company",
            title: "Company",
            order: 1, // Usually first
            // @ts-ignore
            links: [
                { id: "newsletter-static", text: "Newsletter", url: "?newsletter=true", order: 1 }
            ]
        } as any);
    }
    ...
    const tagline = footerSettings?.tagline || "Your 24/7 AI workforce. Content, Marketing, and Growth on autopilot.";
        ...
        <footer className="w-full bg-[#0F0F1A] text-[#94A3B8] font-sans border-t border-gray-800/50">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 py-10 lg:py-14">
                ...
                <div className="flex flex-col lg:flex-row gap-10 lg:gap-0">
                    ...
                    {/* Logo, Tagline, & Socials - Left side */}
                        ...
                        <div className="flex items-center gap-4 pt-2">
                            {socialLinks.map((social, index) => (
                                <a
                                    ...
                                    className={`text-white/60 ${social.hoverColor} hover:scale-110 transition-all duration-200`}
                                >
                                    {social.icon}
                                </a>
                            ))}
                    ...
                    {/* Navigation Sections */}
                    <div className="flex flex-wrap sm:flex-nowrap justify-center lg:justify-between lg:flex-1 gap-8 sm:gap-12 lg:gap-8 px-0 lg:px-8">
                ...
                {/* Middle Section: Contact & Downloads - Horizontal Bar */}
                <div className="mt-16 py-8 border-t border-b border-gray-800/50 flex flex-col md:flex-row justify-between items-center gap-6">
                    {/* Left: Contact Info */}
                    <div className="flex flex-wrap justify-center md:justify-start gap-6 text-sm">
                        {socialSettings?.emailSupport && (
                            <a href={`mailto:${socialSettings.emailSupport}`} className="hover:text-white transition-colors flex items-center gap-2 text-white/70 group">
                        ...
                        {socialSettings?.emailSales && (
                            <a href={`mailto:${socialSettings.emailSales}`} className="hover:text-white transition-colors flex items-center gap-2 text-white/70 group">
                                <div className="p-2 rounded-full bg-white/5 group-hover:bg-white/10 transition-colors">
                                    <FaEnvelope className="w-4 h-4 text-white/70" />
                                </div>
                        ...
                        {socialSettings?.phoneNumber && (
                            <a href={`tel:${socialSettings.phoneNumber}`} className="hover:text-white transition-colors flex items-center gap-2 text-white/70 group">
                                <div className="p-2 rounded-full bg-white/5 group-hover:bg-white/10 transition-colors">
                    ...
                    {/* Right: App Downloads */}
                    {(socialSettings?.appStoreUrl || socialSettings?.playStoreUrl) && (
                        <div className="flex items-center gap-4">
                            {socialSettings?.appStoreUrl && (
                                <a href={socialSettings.appStoreUrl} target="_blank" rel="noopener noreferrer" className="opacity-80 hover:opacity-100 hover:scale-105 transition-all">
                                    <Image src="https://upload.wikimedia.org/wikipedia/commons/3/3c/Download_on_the_App_Store_Badge.svg" alt="Download on App Store" width={110} height={35} className="h-8 w-auto" unoptimized />
                                </a>
                            )}
                            {socialSettings?.playStoreUrl && (
                                <a href={socialSettings.playStoreUrl} target="_blank" rel="noopener noreferrer" className="opacity-80 hover:opacity-100 hover:scale-105 transition-all">
                                    <Image src="https://upload.wikimedia.org/wikipedia/commons/7/78/Google_Play_Store_badge_EN.svg" alt="Get it on Google Play" width={125} height={35} className="h-8 w-auto" unoptimized />
                                </a>
                            )}
                        </div>
                    )}
