...
/**
 * POST /api/integration/echo/register
 * Registers Echo connection details inside CRM for UI status indicators.
 *
 * Expects:
 *  - Header: x-wallet (required)
 *  - Body: { serviceUrl?: string }  // optional Echo base/origin
 *
 * Behavior:
 *  - Upserts systemServices entry with name='echo'
...
export async function POST(req: Request) {
  try {
    ...
    // Fallback: allow Echo to pass the CRM user id explicitly via header/body when session is absent
    try {
    ...
    const existing = await prismadb.systemServices.findFirst({ where: { name: "echo" } });
    if (existing?.id) {
      ...
      const data: any = { name: "echo", serviceId: wallet, v: 0 };
      if (serviceUrl) data.serviceUrl = serviceUrl;
      // Persist wallet→CRM user mapping even if session-less (from header/body fallback)
      if (userId) data.servicePassword = userId;
    ...
    if (userId) {
      try {
        ...
        const updatedResourceLinks = { ...rl, echoWallet: wallet };
